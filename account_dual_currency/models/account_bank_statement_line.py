...
class AccountBankStatementLine(models.Model):
        ...
        for rec in self:
            ...
            if rec.date:
                ...
                if tasa_fecha:
                    ...
                    tasa_referencia_statement = self.env.company.currency_id_dif.rate
            else:
                tasa_referencia_statement = self.env.company.currency_id_dif.rate
    ...
    @api.depends('tasa_referencia_statement', 'amount', 'currency_id_dif_statement', 'statement_id.currency_id_journal', 'journal_id')
    def _amount_usd(self):
        for rec in self:
            if rec.journal_id.currency_id == rec.currency_id_dif_statement:
                rec.amount_usd_statement = rec.amount
            else:
                rec.amount_usd_statement = (rec.amount / rec.tasa_referencia_statement) if rec.tasa_referencia_statement > 0 else 0
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if 'journal_id' in vals:
                ...
                # buscar la fecha en vals
                if 'date' in vals:
                    domain = [('name', '=', vals['date']), ('currency_id', '=', self.env.company.currency_id_dif.id),
                              ('company_id', '=', self.env.company.id)]
                    # print(domain)
                    tasa_fecha = self.env['res.currency.rate'].search(domain)
                    if tasa_fecha:
                        tasa = tasa_fecha.inverse_company_rate
                    else:
                        tasa = self.env.company.currency_id_dif.inverse_rate
            if not tasa:
                if 'journal_id' in vals:
                    journal = self.env['account.journal'].browse(vals['journal_id'])
                    tasa = journal.company_id.currency_id_dif.rate
    ...
    def write(self, vals):
        ...
        if isinstance(vals, list):
            for val in vals:
                if 'tasa_referencia_statement' in val:
                    val['tax_today'] = val['tasa_referencia_statement']
        else:
        ...
        for rec in self:
            if rec.move_id.journal_id.currency_id:
                if rec.move_id.journal_id.currency_id != rec.company_id.currency_id:
                    rec.move_id._onchange_tax_today()
        return res
