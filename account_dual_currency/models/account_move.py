...
from odoo.addons.account.models.account_move_line import AccountMoveLine as OdooAccountMoveLine
...
class AccountMove(models.Model):
    ...
    tax_today = fields.Float(string="Tasa", store=True,
                             default=lambda self: self.env.company.currency_id_dif.rate,
    ...
    amount_untaxed_usd = fields.Monetary(currency_field='currency_id_dif', string="Base imponible Ref.", store=True,
                                         compute="_amount_all_usd", digits='Dual_Currency', copy=False, precompute=True)
    amount_tax_usd = fields.Monetary(currency_field='currency_id_dif', string="Impuestos Ref.", store=True,
                                     readonly=True, digits='Dual_Currency', compute="_amount_all_usd", copy=False, precompute=True)
    amount_total_usd = fields.Monetary(currency_field='currency_id_dif', string='Total Ref.', store=True, readonly=True,
                                       ...
                                       digits='Dual_Currency', tracking=True, precompute=True)
    ...
    def _post(self, soft=True):
        ...
        for move in self:
            for l in move.line_ids:
                if l.credit_usd == 0 and l.debit_usd == 0:
        ...
        for move in self:
            ...
            if asset:
                ...
                asset_depreciation = sum(
                    move.line_ids.filtered(lambda l: l.account_id.internal_group == account_internal_group or l.account_id == asset.account_depreciation_expense_id).mapped('balance_usd')
                )
                ...
                if any(
                        ...
                        and float_compare(-line.balance_usd, asset.original_value_ref, precision_rounding=asset.currency_id.rounding) == 0
                    ...
                    asset_depreciation = (
                        ...
                        - (
                            move.line_ids[1].debit_usd if asset.original_value_ref > 0 else move.line_ids[1].credit_usd
            ...
            for val in values:
                if not 'tax_today' in val and not diferencia:
                    ...
                    if module_dual_currency:
                        val.update({'tax_today': self.env.company.currency_id_dif.rate})
    ...
    def write(self, vals):
        ...
        #buscar el line_ids lo que no sea payment_term
        for rec in self:
            payment_term = rec.line_ids.filtered(lambda l: l.display_type == 'payment_term')
            if payment_term:
                if payment_term.debit_usd > 0:
                    payment_term.debit_usd = sum(rec.line_ids.filtered(lambda l: l.credit_usd > 0 and l.display_type != 'payment_term').mapped('credit_usd'))
                elif payment_term.credit_usd > 0:
                    payment_term.credit_usd = sum(rec.line_ids.filtered(lambda l: l.debit_usd > 0 and l.display_type != 'payment_term').mapped('debit_usd'))
    ...
    def _onchange_currency(self):
        ...
        for rec in self:
            # Determinar el factor de ajuste para el precio_unit
            factor = rec.tax_today if rec.tax_today > 0 else 1

            # Actualizar todas las líneas de factura en un solo bucle
            for l in rec.invoice_line_ids:
                l.currency_id = rec.currency_id
                if rec.currency_id == company_currency:
                    l.price_unit = rec.currency_id._convert_new(to_currency=rec.currency_id, from_amount=l.price_unit_usd,
                                                             to_rate=1, from_rate=rec.tax_today)
                else:
                    l.price_unit = l.price_unit_usd

                    # Actualizar las líneas contables
            for aml in rec.line_ids:
    ...
    @api.depends('tax_totals', 'currency_id_dif', 'currency_id', 'tax_today', 'state', 'line_ids.balance_usd', 'invoice_line_ids.price_unit_usd',
                 'line_ids.balance', 'line_ids',
                 'line_ids.matched_debit_ids.debit_move_id.move_id.payment_id.is_matched',
                 'line_ids.matched_debit_ids.debit_move_id.move_id.line_ids.amount_residual',
                 'line_ids.matched_debit_ids.debit_move_id.move_id.line_ids.amount_residual_currency',
                 'line_ids.matched_credit_ids.credit_move_id.move_id.payment_id.is_matched',
    ...
    def _amount_all_usd(self):
        for rec in self:
            ...
            if rec.is_invoice(include_receipts=True) and rec.tax_totals:
                rec.line_ids._onchange_amount_currency()
                ...
                amount_tax = sum(x['tax_group_amount'] for x in (rec.tax_totals['groups_by_subtotal']['Subtotal']
                                                        if 'Subtotal' in rec.tax_totals['groups_by_subtotal']
                                                            else rec.tax_totals['groups_by_subtotal']['Base imponible']))
                logger.info(f"Nombre que sale en tax_group_amount {'Subtotal' if 'Subtotal' in rec.tax_totals['groups_by_subtotal'] else 'Base imponible'} - Valor {amount_tax}")
                logger.info(f"Nombre que sale en tax_group_amount {rec.tax_totals['groups_by_subtotal']} - Valor {amount_tax}")
                amount_untaxed = rec.tax_totals['amount_untaxed']


                if rec.currency_id.id == self.env.company.currency_id_dif.id:
                    if amount_tax:
                        amount_tax_new = rec.currency_id._convert_new(to_currency=rec.currency_id, from_amount=amount_tax,
                                                             to_rate=1, from_rate=rec.tax_today)
                    else:
                        amount_tax_new = 0

                    if amount_untaxed:
                        amount_untaxed_new = rec.currency_id._convert_new(to_currency=rec.currency_id, from_amount=amount_untaxed,
                                                             to_rate=1, from_rate=rec.tax_today)
                    else:
                        amount_untaxed_new = 0
                    ...
                    rec.amount_untaxed_bs = amount_untaxed_new
                    rec.amount_tax_bs = amount_tax_new
                    rec.amount_total_bs = rec.amount_untaxed_bs + rec.amount_tax_bs

                elif rec.currency_id.id == self.env.company.currency_id.id:
                    if amount_tax:
                        amount_tax_new = rec.currency_id._convert_new(to_currency=rec.currency_id,
                                                                              from_amount=amount_tax,
                                                                              to_rate=rec.tax_today, from_rate=1)
                    else:
                        amount_tax_new = 0

                    if amount_untaxed:
                        amount_untaxed_new = rec.currency_id._convert_new(to_currency=rec.currency_id,
                                                                                  from_amount=amount_untaxed,
                                                                                  to_rate=rec.tax_today, from_rate=1)
                    else:
                        amount_untaxed_new = 0
                    ...
                    rec.amount_untaxed_usd = amount_untaxed_new
                    rec.amount_tax_usd = amount_tax_new
                    rec.amount_total_usd = rec.amount_untaxed_usd + rec.amount_tax_usd

                ##Mejora de calculos - prueba 1
                # if rec.line_ids and len(rec.invoice_line_ids) == len(rec.line_ids.filtered(lambda line: line.display_type == 'product')):
                #
                #     if len(rec.line_ids.filtered(lambda line: line.display_type == 'product')) > 0:
                #         rec.amount_untaxed_usd = abs(sum(rec.line_ids.filtered(lambda line: line.display_type == 'product').mapped('balance_usd')))
                #     else:
                #         rec.amount_untaxed_usd = 0
                #     if len(rec.line_ids.filtered(lambda line: line.display_type not in ('product', 'payment_term'))) > 0:
                #         rec.amount_tax_usd = abs(sum(rec.line_ids.filtered(lambda line: line.display_type not in ('product', 'payment_term')).mapped('balance_usd')))
                #     else:
                #         rec.amount_tax_usd = 0
                # else:
                #     amount_tax = sum(x['tax_group_amount'] for x in rec.tax_totals['groups_by_subtotal']['Base imponible'])
                #     amount_untaxed = rec.tax_totals['amount_untaxed']
                #
                #     if amount_tax > 0:
                #         rec.amount_tax_usd = rec.currency_id._convert(
                #                                 abs(amount_tax),
                #                                 rec.currency_id_dif,
                #                                 rec.company_id,
                #                                 rec.date,
                ...
                #             rec.amount_untaxed_usd = 0
                #
                #     logger.info('amount_tax_usd %s', rec.amount_tax_usd)
                #     logger.info('amount_tax_usd %s', rec.amount_tax_usd)
                #     logger.info('amount_tax_usd %s', rec.amount_tax_usd)
                #     logger.info('amount_tax_usd %s', rec.amount_tax_usd)
                #     logger.info(rec)
                ...
                #     rec.amount_tax_bs = rec.amount_tax or 0
    ...
    def _check_balanced(self, container):
        ...
        if unbalanced_moves:
            ...
            for move_id, sum_debit, sum_credit in unbalanced_moves:
                decimal_difference = round(sum_debit - sum_credit, 2)
                if abs(decimal_difference) < 0.05:
                    self._balanced_move_lines(self.line_ids, decimal_difference)
                    continue
                else:
    ...
    def _balanced_move_lines(self, line_ids, amount_rest_balance):
        balance_rest = abs(amount_rest_balance)
        for line in line_ids:
            if balance_rest > 0.00:
                if amount_rest_balance > 0 and line.debit:
                    query = """
                        UPDATE account_move_line 
                        SET debit = %s
                        WHERE id = %s"""
                    self._cr.execute(query, [line.debit - 0.01, line.id])
                    balance_rest -= 0.01
                if amount_rest_balance < 0 and line.credit:
                    query = """
                        UPDATE account_move_line 
                        SET credit = %s
                        WHERE id = %s"""
                    self._cr.execute(query, [line.credit - 0.01, line.id])
                    balance_rest -= 0.01
                self._cr.execute("""
                     UPDATE account_move_line 
...
def _sync_invoice(self, container):
    ...
    for line in after:
        if (
            ...
            if line.currency_id == line.company_id.currency_id:
                ...
                if line.tax_today:
                    line.balance = line.currency_id._convert_new(to_currency=line.currency_id, from_amount=line.amount_currency, to_rate=1, from_rate=line.tax_today)
                else:
                    line.balance = line.currency_id._convert_new(to_currency=line.currency_id,
                                                                 from_amount=line.amount_currency, to_rate=1,
                                                                 from_rate=line.move_id.tax_today)
    ...
    for line in after:
        if (
            ...
            # Se modifico el campo currency_rate por tax_today
            # balance = line.company_id.currency_id.round(line.amount_currency / line.currency_rate)
            if line.currency_id == line.company_id.currency_id:
                balance = line.company_id.currency_id.round(line.amount_currency)
            else:
                balance = line.currency_id._convert_new(to_currency=line.currency_id, from_amount=line.amount_currency, to_rate=1, from_rate=line.tax_today)
                # balance = line.company_id.currency_id.round(line.amount_currency * line.tax_today)
    ...
    #
    # unbalanced_moves = self.move_id._get_unbalanced_moves({'records': self.move_id})
    # if unbalanced_moves:
    #     for move_id, sum_debit, sum_credit in unbalanced_moves:
    #         decimal_difference = round(sum_debit - sum_credit, 2)
    #         if abs(decimal_difference) < 0.05:
    #             self.move_id._balanced_move_lines(after, decimal_difference)


AM._post = _post
OdooAccountMoveLine._sync_invoice = _sync_invoice
