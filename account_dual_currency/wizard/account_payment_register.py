...
class AccountPaymentRegister(models.TransientModel):
        ...
        for wizard in self:
            ...
            if wizard.source_currency_id == wizard.currency_id:
                ...
                if wizard.currency_id == wizard.company_id.currency_id_dif:
                    payment_difference_usd = round(wizard.amount_residual_usd - wizard.amount, 4)
                    wizard.payment_difference_usd = payment_difference_usd
                    ...
                    # wizard.payment_difference_bs = payment_difference_usd * (wizard.tax_today if wizard.tax_today else 1 )
                    # wizard.payment_difference_bs = (wizard.amount_residual_usd / wizard.tax_invoice) - (wizard.amount / (wizard.tax_today if wizard.tax_today > 0 else 1))
                ##print('diferencia 1')
            elif wizard.currency_id == wizard.company_id.currency_id:
                # Payment expressed on the company's currency.
    ...
    def _get_wizard_values_from_batch(self, batch_result):
        ...
        if not self.tax_today:
            tax_today = lines[0].company_id.currency_id_dif.rate
        else:
    ...
    def _create_payments(self):
        ...
        if self.source_amount == 0:
            ...
            if not self.currency_id.is_zero(self.payment_difference) and self.payment_difference_bs < 0 and self.payment_difference_handling == 'reconcile':
                ...
                #Validacion de montos por diferenciales
                amount_residual_line = self.line_ids[0].amount_residual
                tolerancia = 0.02
                if abs(self.payment_difference_bs - amount_residual_line) <= tolerancia:
                    self.payment_difference_bs = amount_residual_line
            ...
            elif (self.payment_difference > 0 and self.payment_difference_usd > 0 and self.payment_difference_handling == 'reconcile'
