...
class AccountMove(models.Model):
    ...
    #retencion municipal
    municipal_rate_id = fields.Many2one('account.wh.municipal.rates', string='Concepto de retención municipal', copy=False)
    rate_value = fields.Float(string='Alícuota', related='municipal_rate_id.rate', store=True, copy=False)
    monto_alicuota = fields.Monetary(string='Monto de la alícuota', copy=False, store=True, currency_field='company_currency_id', compute='_compute_monto_alicuota', tracking=True)
    porc_reten_muni = fields.Float(string='Porcentaje de retención municipal', copy=False, store=True, default=100.0)
    monto_reten_muni = fields.Monetary(string='Monto de retención municipal', copy=False, store=True, currency_field='company_currency_id', compute='_compute_monto_reten_muni', tracking=True)
    account_move_ret_muni_id = fields.Many2one('account.move', string='Asiento retención municipal', copy=False, readonly=True, force_save=True)
    wh_muni_id = fields.Many2one('account.wh.municipal.docs', string='Documento de retención municipal', copy=False)

    reversed_entry_id_new = fields.Many2one(
    ...
    def action_post(self):
        ...
        for res in self:
            if res.move_type in ['out_invoice', 'out_refund']:
                if not res.nro_ctrl:
                    res.nro_ctrl = res._get_sequence_code()

                    numeros_control_usados = self.env['account.move'].search(
                        [('move_type', 'in', ['out_invoice', 'out_refund']), ('nro_ctrl', '=', res.nro_ctrl),
                         ('state', '=', 'posted'), ('id', '!=', res.id)])
                    if numeros_control_usados:
                        raise UserError(_('El número de control ya se encuentra asignado a un factura confirmada.\n'
                                          'Por favor validar con el administrador'))
                    res.write({'nro_ctrl': res.nro_ctrl})
    ...
    def create(self, values):
        ...
        if values:
            for val in values:
                if 'reversed_entry_id_new' in val:
                    val['reversed_entry_id'] = val['reversed_entry_id_new']
    ...
    def write(self, vals):
        if 'reversed_entry_id_new' in vals:
            vals['reversed_entry_id'] = vals['reversed_entry_id_new']
        # Se comentó porque la escritura constante estaba dejando el write en un bucle que daba error
        ...
        if vals.get('supplier_invoice_number', False):
            ...
            if not supplier_invoice_number_id:
                ...
                return {'UserError': {'title': "Advertencia!",
        ...
        # Se comentó ya que el código no estaba haciendo nada, la validación de abajo ya estaba comentada previamente
    ...
    def _create_islr_wh_doc(self):
        ...
        if wh_ret_code:
            journal = None #wh_doc_obj._get_journal(self.partner_id)
    ...
    @api.depends('municipal_rate_id', 'monto_alicuota', 'porc_reten_muni', 'state', 'company_id.aplicar_retencion_municipal')
    def _compute_monto_reten_muni(self):
        for rec in self:
            if rec.move_type in ('in_invoice', 'in_refund') and rec.company_id.aplicar_retencion_municipal:
                #verificar que porc_reten_muni sea mayor a 0 y monto_alicuota sea mayor a 0
                if rec.porc_reten_muni > 0 and rec.monto_alicuota > 0:
                    rec.monto_reten_muni = (rec.monto_alicuota * rec.porc_reten_muni) / 100
                else:
                    rec.monto_reten_muni = 0
            elif rec.move_type in ('out_invoice', 'out_refund') and rec.partner_id.agente_retencion_mun:
                if rec.porc_reten_muni > 0 and rec.monto_alicuota > 0:
                    rec.monto_reten_muni = (rec.monto_alicuota * rec.porc_reten_muni) / 100
                else:
                    rec.monto_reten_muni = 0
    ...
    @api.depends('amount_untaxed', 'amount_tax', 'amount_total', 'rate_value', 'municipal_rate_id', 'state',
                 'porc_reten_muni')
    def _compute_monto_alicuota(self):
        for rec in self:
            if rec.municipal_rate_id:
                if rec.company_id.aplicar_sobre == 'amount_untaxed':
                    rec.monto_alicuota = rec.amount_untaxed * rec.rate_value / 100
                elif rec.company_id.aplicar_sobre == 'amount_total':
                    rec.monto_alicuota = rec.amount_total * rec.rate_value / 100
            else:
                rec.monto_alicuota = 0

    def cargar_licencias(self):
        for rec in self:
            #buscar todos los contactos que tengas rif
            partners = self.env['res.partner'].search([('rif', '!=', False)])
            for partner in partners:
                pass

    def descargar_alicuotas(self):
        for rec in self:
            ...
            for i in range(1, 15000):
                ...
                if response.text:
                    ...
                    if data.get('data'):
                        municipal_rate_ids = []
                        print(f"Registro {i} cargado", data['data'])
                        if 'rubros' in data['data']:
                            for rubros in data['data']['rubros'][0]:
                                ...
                                if 'codigo' in rubros:
                                    ...
                                    if not code == None and ordenanza == 'Ordenanza Marzo 2024':
    ...
    def generate_wh_muni(self):
        for rec in self:

            #crear asiento contable con el monto de monto_reten_muni
            if rec.monto_reten_muni > 0 and not rec.account_move_ret_muni_id:
                journal_id = 0
                account_id = 0
                if rec.move_type in ('in_invoice', 'in_refund'):
                    journal_id = rec.partner_id.journal_muni_purchase_id.id or rec.company_id.journal_muni_purchase_id.id
                    account_id = rec.company_id.account_debit_wh_municipal_id.id
                elif rec.move_type in ('out_invoice', 'out_refund'):
                    journal_id = rec.partner_id.journal_muni_sale_id.id or rec.company_id.journal_muni_sale_id.id
                    account_id = rec.company_id.account_credit_wh_municipal_id.id

                #buscar la cuenta contable en line_ids si es pagable o recibible
                account_invoice_id = rec.line_ids.filtered(lambda line: line.account_id.account_type == 'asset_receivable' or line.account_id.account_type == 'liability_payable')
                ...
                move = account_move.create({
                    ...
                    'ref': 'RET MUN %s' % rec.name,
                    'company_id': rec.company_id.id,
                    'move_type': 'entry',
                })

                #verificar si esta instalado el módulo account_dual_currency
                module_dual_currency = self.env['ir.module.module'].sudo().search([('name','=','account_dual_currency'),('state','=','installed')])
                if module_dual_currency:
                    move.tax_today = rec.tax_today

                if rec.move_type in ('in_invoice', 'in_refund'):
                    #crear la linea de asiento contable
                    move.write({
                        'line_ids': [(0, 0, {
                            'name': rec.name,
                            'move_id': move.id,
                            'account_id': account_invoice_id.account_id.id,
                            'debit': rec.monto_reten_muni,
                            'credit': 0.0,
                            'partner_id': rec.partner_id.id,
                            'date_maturity': rec.invoice_date,
                            'date': rec.invoice_date,
                ...
                move.action_post()
                rec.account_move_ret_muni_id = move.id

                #conciliar el asiento contable con la factura
                line_ids = rec.line_ids.filtered(lambda line: line.account_id.account_type == 'asset_receivable' or line.account_id.account_type == 'liability_payable')
                line_move_ids = move.line_ids.filtered(lambda line: line.account_id.account_type == 'asset_receivable' or line.account_id.account_type == 'liability_payable')
                (line_ids + line_move_ids).reconcile()
